        ...
        <div className="container">
          ...
          <div className="product-grid">
            {products.map((product) => {
              ...
              return (
                <div key={product.name} className="product-card" style={{ opacity: isOutOfStock ? 0.6 : 1 }}>
                  <div className="product-image" style={{ position: "relative", height: 180, overflow: "hidden", background: "#111" }}>
                    <Image src={product.image} alt={product.name} fill style={{ objectFit: "contain", padding: "8px" }} sizes="(max-width: 640px) 50vw, 33vw" />
                    <div className="product-discount">{product.discount}</div>
                    <div className="product-puffs">{product.puffs}</div>
                    {isOutOfStock && (
                      <div style={{ position: "absolute", inset: 0, background: "rgba(0,0,0,0.6)", display: "flex", alignItems: "center", justifyContent: "center" }}>
                        <span style={{ color: "#ef4444", fontFamily: "var(--font-display)", fontWeight: 800, fontSize: "0.9rem" }}>STOCK OUT</span>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="product-info" style={{ padding: "10px 10px 12px" }}>
                    ...
                    {!tag && <div className="product-badge">{product.badge}</div>}
                    ...
                    {selectedFlavour && (
                      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", background: "var(--orange-glow)", border: "1px solid rgba(255,92,0,0.3)", borderRadius: 6, padding: "4px 8px", marginBottom: 8 }}>
                    ...
                    <button
                      ...
                      style={{
                        ...
                        background: isOutOfStock ? "var(--bg-3)" : justAdded ? "#10b981" : selectedFlavour ? "var(--orange)" : "var(--bg-3)",
                        border: (isOutOfStock || selectedFlavour) ? "none" : "1px solid var(--border)",
                        borderRadius: 8,
                        color: isOutOfStock ? "var(--muted)" : selectedFlavour ? "#fff" : "var(--muted)",
                        fontFamily: "var(--font-display)", fontWeight: 700,
                        fontSize: "0.85rem", cursor: isOutOfStock ? "not-allowed" : "pointer",
                        transition: "all 0.2s", WebkitTapHighlightColor: "transparent",
                      }}
                    >
                      {isOutOfStock ? "Out of Stock" : justAdded ? "Added!" : selectedFlavour ? "+ Add to Cart" : "Select Flavour"}
                    </button>
                  </div>
                </div>
              );
        ...
        <>
          ...
          <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, background: "var(--bg-2)", borderRadius: "20px 20px 0 0", border: "1px solid var(--border)", borderBottom: "none", zIndex: 10003, padding: "20px 20px 40px", maxHeight: "70vh", overflowY: "auto" }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
              ...
              <button onClick={() => setFlavourSheet(null)} style={{ background: "var(--bg-3)", border: "1px solid var(--border)", color: "var(--muted)", borderRadius: 8, padding: "6px 10px", cursor: "pointer", fontSize: "0.85rem" }}>✕</button>
            </div>
              ...
              {(getOverride(sheetProduct.name)?.flavours?.length ? getOverride(sheetProduct.name)!.flavours : sheetProduct.flavours).map(flavour => {
                ...
                return (
                  <button key={flavour} onClick={() => setSelectedFlavours(prev => ({ ...prev, [sheetProduct.name]: flavour }))}
                    style={{ padding: "10px 12px", borderRadius: 10, textAlign: "left", border: isSelected ? "1.5px solid var(--orange)" : "1px solid var(--border)", background: isSelected ? "var(--orange-glow)" : "var(--bg-3)", color: isSelected ? "var(--orange)" : "var(--text)", fontSize: "0.82rem", fontFamily: "var(--font-body)", fontWeight: 500, cursor: "pointer", WebkitTapHighlightColor: "transparent" }}>
                    {flavour}
                  </button>
                );
              })}
            ...
            <button onClick={() => handleAddToCart(sheetProduct)} disabled={!selectedFlavours[sheetProduct.name]}
              style={{ width: "100%", padding: "14px", background: selectedFlavours[sheetProduct.name] ? "var(--orange)" : "var(--bg-3)", border: "none", borderRadius: 10, color: "#fff", fontFamily: "var(--font-display)", fontWeight: 700, fontSize: "1rem", cursor: selectedFlavours[sheetProduct.name] ? "pointer" : "not-allowed", opacity: selectedFlavours[sheetProduct.name] ? 1 : 0.5, WebkitTapHighlightColor: "transparent" }}>

...
export default function ProductGrid() {
  ...
  const sortedProducts = [...products].sort((a, b) => {
    const aOut = (getOverride(a.name) && !getOverride(a.name)!.in_stock);
    const bOut = (getOverride(b.name) && !getOverride(b.name)!.in_stock);
    if (aOut && !bOut) return 1;
    if (!aOut && bOut) return -1;
    const aIdx = productOrder.indexOf(a.name);
    const bIdx = productOrder.indexOf(b.name);
  ...
  const filteredProducts = filterFn ? sortedProducts.filter(p => filterFn(p.name)) : sortedProducts;
  ...
  return (
    <>
      ...
      {showWA && (
        <a
          href={`https://wa.me/${ADMIN_PHONE}?text=Hi%2C%20I%20need%20help%20with%20my%20order%20in%20Bangalore`}
          target="_blank"
      ...
      <section className="section" id="products" style={{ paddingTop: 20 }}>
        <div className="container">
          ...
          <p style={{ marginBottom: 14 }}>Select your flavour and add to cart — delivered in 30-45 mins.</p>
              ...
              const card = (
                ...
                >
                  ...
                  <div className="product-info" style={{ padding: "10px 10px 12px", display: "flex", flexDirection: "column", flex: 1 }}>
                    {/* Badge */}
                    {(tag && tagStyle) ? (
                      <div style={{ display: "inline-block", background: tagStyle.bg, color: tagStyle.color, border: `1px solid ${tagStyle.color}44`, borderRadius: 20, padding: "2px 8px", fontSize: "0.65rem", fontWeight: 700, fontFamily: "var(--font-display)", marginBottom: 5, alignSelf: "flex-start" }}>
                        {tag}
                      </div>
                    ) : product.badge ? (
